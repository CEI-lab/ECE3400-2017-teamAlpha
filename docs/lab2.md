...
Second, we read through the example code provided with the Open Music Labs FFT library (fft_adc_serial.pde). This code interacts with the Arduino's ADC directly, bypassing the interface provided by analogRead(). In theory, this allows use to sample from the ADC faster than the analogRead().



[Note: Kirstin, include information on the amplifier that you used along with a diagram.]
